...
Options:
        ...
        --only WHICH
                Downloads only audio or only video.
...
func init() {
    ...
    flagSet.Func("only", "Choose to download only audio or video.", func(s string) error {
        switch s {
        case "audio":
            if onlyVideo {
                return fmt.Errorf("Both --only audio and --only video present")
            }
            onlyAudio = true
        case "video":
            if onlyAudio {
            ...
            onlyVideo = true
        default:
            return fmt.Errorf("Argument must be either 'audio' or 'video', got '%s'", s)
        }
        return nil
    })
